...
library(Seurat)
d.integrated <- readRDS(paste0('./brain/atlas/seurat/result/',data,'/integrated.rds'))
library(ggplot2)
library(cowplot)
...
dir.create(paste0('./brain/atlas/seurat/result/',data,'/pca/'),showWarnings = F,recursive = T)
saveRDS(pca,paste0('./brain/atlas/seurat/result/',data,'/pca/pca.rds'))

d.integrated <- RunUMAP(d.integrated, reduction = "pca", dims = 1:15)
u = d.integrated@reductions$umap@cell.embeddings
dir.create(paste0('./brain/atlas/seurat/result/',data,'/umap_15PC/'),showWarnings = F,recursive = T)
saveRDS(u,paste0('./brain/atlas/seurat/result/',data,'/umap_15PC/umap.rds'))
