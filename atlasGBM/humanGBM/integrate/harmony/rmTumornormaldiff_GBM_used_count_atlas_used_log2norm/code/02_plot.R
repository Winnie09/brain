...
pdir <- './humanGBM/integrate/harmony/rmTumornormaldiff/plot/'  ####
u <- readRDS('./humanGBM/integrate/harmony/rmTumornormaldiff/res/umap_embeddings.rds') ########
...
##
  ...
  p <- ggplot() + geom_scattermore(data=data.frame(umap1=u[,1],umap2=u[,2],feature=ifelse(id.gbm, 'GBM', 'atlas')),aes(x=umap1,y=umap2,col=feature),alpha=0.2,size=0.1) + 
    ...
    theme(legend.spacing.x = unit(-0.1, 'cm'),legend.spacing.y = unit(-0.3, 'cm'))+
     scale_color_manual(values=c('orange', 'steelblue')) +
    facet_wrap(~feature)
  ggsave(paste0(pdir,'umap_GBM_atlas_facet.png'),p,height=4, width=5,dpi=200)
