...
library(here)
setwd(here())
k <- readRDS('atlasGBM/humanGBM/integrate/harmony/36nonNormal_allen10xOnly_seuratGene/res/atlasGBM_harmony.rds')
...
for (rid in sort(unique(clu))) {
  ...
  if (sum(tab ==2) > 1) {
    ...
    dir.create('atlasGBM/humanGBM/integrate/harmony/36nonNormal_allen10xOnly_seuratGene/diff/')
    saveRDS(res,file=paste0('atlasGBM/humanGBM/integrate/harmony/36nonNormal_allen10xOnly_seuratGene/diff/',rid,'.rds'))
  }
}
