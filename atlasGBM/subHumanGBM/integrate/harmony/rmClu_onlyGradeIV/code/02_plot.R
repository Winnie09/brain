library(here)
setwd(here())
ddir <- 'atlasGBM/subHumanGBM/integrate/harmony/rmClu_onlyGradeIV/result/'
pdir <- 'atlasGBM/subHumanGBM/integrate/harmony/rmClu_onlyGradeIV/plot/'
u <- readRDS(paste0(ddir, 'sub_umap_embeddings.rds'))
...
####################### Seurat plot
seu <- readRDS(paste0(ddir, 'sub_atlasGBM_harmony.rds'))
gbm <- readRDS('atlasGBM/GBMonly/integrate/harmony/36nonNormalSeuratGene2000/res/celltype_annotation.rds')
atlas <- readRDS('atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/res/celltype_annotation.rds')
anno <- c(atlas, gbm)
meta$celltype_anno <- anno[sub('\\(.*', '', meta$cell)]
seu@meta.data <- meta

p <- DimPlot(seu, group.by = 'celltype_anno', split.by = 'celltype_anno', ncol = 4)
library(ggplot2)
ggsave(paste0(pdir, 'umap_celltype_anno_facet.png'), p, width = 15, height = 15, dpi = 200)

p <- DimPlot(seu, group.by = 'celltype_anno')
ggsave(paste0(pdir, 'umap_celltype_anno.png'), p, width = 9, height = 3.5, dpi = 200)
