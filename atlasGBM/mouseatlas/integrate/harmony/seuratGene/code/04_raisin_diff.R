...
library(here)
setwd(here())
k <- readRDS('atlasGBM/mouseatlas/integrate/harmony/seuratGene/res/humanAtlas_harmony.rds')
...
for (rid in sort(unique(clu))) {
  ...
  if (sum(tab ==2) >1) {
    ...
    dir.create('atlasGBM/mouseatlas/integrate/harmony/seuratGene/diff/')
    saveRDS(res,file=paste0('atlasGBM/mouseatlas/integrate/harmony/seuratGene/diff/',rid,'.rds'))
    write.csv(res,file=paste0('atlasGBM/mouseatlas/integrate/harmony/seuratGene/diff/',rid,'.csv'), row.names = TRUE)
  } else {
    select <- d[, cid]
    select.type <- ctype[cid]
    fc <- rowMeans(select[, grepl('TRUE', select.type), drop = FALSE]) - rowMeans(select[, grepl('FALSE', select.type), drop = FALSE]) 
    fc <- rev(sort(fc))
    res <- data.frame(Foldchange = fc, stringsAsFactors = FALSE)
    rownames(res) <- names(fc)
    saveRDS(res,file=paste0('atlasGBM/mouseatlas/integrate/harmony/seuratGene/diff/',rid,'.rds'))
    write.csv(res,file=paste0('atlasGBM/mouseatlas/integrate/harmony/seuratGene/diff/',rid,'.csv'), row.names = TRUE)
  }
}
