...
here()
setwd(here())
rdir <- ddir <- 'atlasGBM/mouseatlas/integrate/harmony/seuratGene/res/'
pdir <- 'atlasGBM/mouseatlas/integrate/harmony/seuratGene/plot/diffusion_from_harmony_sub/'
...
str(dm)
saveRDS(dm, paste0(rdir, 'diffusionMap.obj_from_harmony_sub.rds'))
dmap <- dm@eigenvectors
rownames(dmap) <- colnames(expr)
str(dmap)
saveRDS(dmap, paste0(rdir, 'diffusionMap_from_harmony_sub.rds'))
