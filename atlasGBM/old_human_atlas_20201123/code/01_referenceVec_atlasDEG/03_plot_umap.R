...
# key = 'referenceVec_atlasDEG'
setwd('/home-4/whou10@jhu.edu/scratch/Wenpin/brain')
datafn = paste0('./human_atlas/result/',key,'/GBM_integratedUMAP.rds')
plotdir = paste0('./human_atlas/plot/',key,'/umap/')
...
# u = u[meta$cell,]
u = readRDS(paste0('./human_atlas/result/',key,'/umap/umap.rds'))
meta = readRDS(paste0('./human_atlas/result/',key,'/meta.rds'))
library(ggplot2)
library(cowplot)
...
ggplot() + geom_point(data=pd,aes(x=umap1,y=umap2,col=density),alpha=0.5,size=0.2)+
  theme_classic() + xlab('UMAP1')+ylab('UMAP2')+
  theme(legend.position = 'right',legend.title = element_blank()) +
  guides(color = guide_legend(override.aes = list(size = 4,alpha=1)))+
  theme(legend.spacing.x = unit(-0.1, 'cm'),legend.spacing.y = unit(-0.3, 'cm'))+
