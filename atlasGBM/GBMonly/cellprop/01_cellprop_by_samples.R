...
tb <- tb/rowSums(tb)
rowSums(tb)
saveRDS(tb, 'atlasGBM/GBMonly/cellprop/cellprop_by_samples.rds')
...
locpal <- brewer.pal(4,'Set1')
names(locpal) <- unique(m$Location)
grdpal <- brewer.pal(3,'Set1')
names(grdpal) <- unique(m$Tumor.Grade)
pathopal <- brewer.pal(4,'Set1')
names(pathopal) <- unique(m$Pathology)
trtpal <- c('red','blue')
names(trtpal) <- unique(m$Treatment)
colv <- colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100)
colv[1:2] <- c('#000000', '#000080')
mybreak = c(0, seq(0.001, max(tb), length.out = 98))

sexpal = c('lightpink', 'lightblue')
names(sexpal) = c('F', 'M')
mutpal = c('lightpink', 'lightblue')
names(mutpal) = c('mutated', 'not mutated')
amppal = c('lightpink', 'lightblue')
names(amppal) = c('amplified', 'not amplified')
losspal = c('lightpink', 'lightblue')
names(losspal) = c('homozygous loss', 'no loss')

annotation_colors = list(Location=locpal,
                         Tumor.Grade=grdpal,
                         Treatment=trtpal, 
                         Pathology = pathopal,
                         Sex = sexpal, 
                         PTEN.mutation = mutpal,
                         EGFR.mutation = mutpal,
                         TP53.mutation = mutpal,
                         EGFR.amplification = amppal,
                         PDGFRA.amplification = amppal,
                         CDKN2A..2G.loss = losspal)
saveRDS(list(colv = colv, annotation_row = m, annotation_colors = annotation_colors), 'atlasGBM/GBMonly/cellprop/color_scheme.rds')
...
pdf('atlasGBM/GBMonly/cellprop/cellprop_by_samples.pdf', width = 10, height = 11)
pheatmap(mat = tb,
         color = colv,
         annotation_row = m, 
...
pdf('atlasGBM/GBMonly/cellprop/celltype_meta_association.pdf', width = 5, height = 4.2)
