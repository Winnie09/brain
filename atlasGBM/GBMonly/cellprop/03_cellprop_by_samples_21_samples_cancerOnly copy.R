...
m <- m[, c(4:7, 10:18)]
m <- m[m['Tumor.Grade'] == 'IV' & m[, 'Treatment'] == 'Untreated', ]
m <- m[, c(-3, -4, -5)]
tb = tb[rownames(m), ]
...
pdf('atlasGBM/GBMonly/cellprop/cellprop_by_samples_21sample_cancerOnly.pdf', width = 10, height = 11)
pheatmap(mat = tb,
         color = colv,
         annotation_row = m, 
         annotation_colors = list(Location=locpal,Tumor.Grade=grdpal,Treatment=trtpal, Pathology = pathopal),
         cluster_cols = T, 
         border_color = NA,
         breaks = mybreak)
dev.off()
...
pdf('atlasGBM/GBMonly/cellprop/celltype_meta_association_21sample_cancerOnly.pdf', width = 5, height = 4.2)
