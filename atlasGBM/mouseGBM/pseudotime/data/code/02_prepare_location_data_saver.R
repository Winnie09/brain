...
resdata = res@assays$RNA@data
data <- readRDS('/home-4/whou10@jhu.edu/work-zfs/whou10/GBM/data/imp/saver/combine/log2norm_matrix_qc.rds')
meta <- res@meta.data
...
for (i in 1:3){
  ...
  for (j in (i+1):length(loc)){
    print(j)
    rdir <- paste0('atlasGBM/mouseGBM/pseudotime/data/testvar_saver/',traj,'/', loc[i], '_', loc[j])
    ...
    saveRDS(expr, paste0(rdir, '/saverlog.rds'))
