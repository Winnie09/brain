...
alltraj = list.files('/home-4/whou10@jhu.edu/scratch/Wenpin/brain/atlasGBM/mouseGBM/pseudotime/res/testvar_pm/')
for (traj in alltraj){
  print(traj)
  allcomparison = list.files(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/brain/atlasGBM/mouseGBM/pseudotime/res/testvar_pm/', traj))
  for (comparison in allcomparison){
    print(comparison)
    rdir <- paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/brain/atlasGBM/mouseGBM/pseudotime/res/testvar_pm/', traj, '/', comparison)
    
    if (file.exists(paste0(rdir, '/res.rds'))){
      pdir <- paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/brain/atlasGBM/mouseGBM/pseudotime/plot/testvar_pm/', traj, '/', comparison)
      ...
      if (length(diffgene) > 0){
        ...
        ## --------------
        allg <- diffgene
        res <- data.frame(gene = allg, stat[allg, ,drop=F], cluster = Res$cluster[allg], stringsAsFactors = F)
