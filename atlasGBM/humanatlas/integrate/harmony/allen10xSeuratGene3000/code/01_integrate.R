...
rdir = './atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene3000/res/'  ##### 
dir.create(rdir, recursive = TRUE, showWarnings = FALSE)
mat =  readRDS('./atlasGBM/humanatlas/data/combine_mat.rds')
meta =  readRDS('./atlasGBM/humanatlas/data/meta_allcell.rds')
rownames(meta) <- meta$cell
selectcell <- meta$cell[!grepl('2014_Pollen_NatBiotech', meta$study) & !grepl('allen_human_fluidigm', meta$study)]
mat <- mat[,selectcell]
mat <- mat[, colSums(mat > 0) > 500]
meta <- meta[colnames(mat), ]
nfeatures <- 3000

meta$mydonor <- paste0(meta$study, ';',meta$gender,';', meta$age, ';',meta$donor)
tab <- table(meta$mydonor)
n <- names(tab)[tab > 50]
id <- which(meta$mydonor %in% n)
meta <- meta[id,]
mat <- mat[,id]
