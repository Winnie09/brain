...
rdir = './atlasGBM/humanatlas/integrate/harmony/bothAllenCtDiffGene/res/'  ##### 
dir.create(rdir, recursive = TRUE, showWarnings = FALSE)
mat =  readRDS('./atlasGBM/humanatlas/data/combine_mat.rds')
meta =  readRDS('./atlasGBM/humanatlas/data/meta_allcell.rds')
rownames(meta) <- meta$cell
selectcell <- sub('.*;', '', meta$cell[!grepl('2014_Pollen_NatBiotech', meta$study)])
mat <- mat[,selectcell]
...
# v = v[v!='']
integrated.features <- readRDS(here('atlasGBM','humanatlas','diffgene','res', 'celltype_diffgene_pairwise_limma_includeBothAllen.rds'))
print(length(integrated.features))
print(sort(integrated.features))
