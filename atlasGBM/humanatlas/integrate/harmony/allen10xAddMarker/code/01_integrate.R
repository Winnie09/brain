...
rdir = './atlasGBM/humanatlas/integrate/harmony/allen10xAddMarker/res/'  ##### 
dir.create(rdir, recursive = TRUE, showWarnings = FALSE)
mat =  readRDS('./atlasGBM/humanatlas/data/combine_mat.rds')
meta =  readRDS('./atlasGBM/humanatlas/data/meta_allcell.rds')
rownames(meta) <- meta$cell
selectcell <- sub('.*;', '', meta$cell[!grepl('2014_Pollen_NatBiotech', meta$study) & !grepl('allen_human_fluidigm', meta$study)])
mat <- mat[,selectcell]
...
v = v[v!='']
integrated.features = unique(c(integrated.features, v))
print(length(integrated.features))
