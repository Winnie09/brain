...
library(here)
setwd(here())
k <- readRDS('atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/res/humanAtlas_harmony.rds')
...
for (rid in sort(unique(clu))) {
  ...
  if (sum(tab == 2) > 1) {
    ...
    dir.create('atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/diff/')
    saveRDS(res,file=paste0('atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/diff/',rid,'.rds'))
    write.csv(res,file=paste0('atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/diff/',rid,'.csv'), row.names = FALSE) 
 }
}
