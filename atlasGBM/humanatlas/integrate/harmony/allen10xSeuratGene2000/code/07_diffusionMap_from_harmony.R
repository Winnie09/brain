...
here()
setwd(here())
rdir <- ddir <- 'atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/res/'
pdir <- 'atlasGBM/humanatlas/integrate/harmony/allen10xSeuratGene2000/plot/diffusion_from_harmony/'
dir.create(pdir, recursive = T)

# read in data, filtering
seu <- readRDS(paste0(ddir, 'atlasGBM_harmony_with_ct.rds'))
...
str(dm)
saveRDS(dm, paste0(rdir, 'diffusionMap.obj_from_harmony.rds'))
dmap <- dm@eigenvectors
rownames(dmap) <- colnames(expr)
str(dmap)
saveRDS(dmap, paste0(rdir, 'diffusionMap_from_harmony.rds'))
